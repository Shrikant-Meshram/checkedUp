  ...
  return (
    <section className="mx-auto max-w-[1500px] space-y-4 lg:space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 sm:hidden">
        <h1 className="type-primary-heading-h1-medium text-foreground">{isPatient ? 'My Reports' : 'Reports'}</h1>
        {isPatient && (
      ...
      <div className="hidden sm:flex items-center justify-between gap-3">
        <div>
          <h1 className="type-primary-heading-h1-medium text-foreground">{isPatient ? 'My Reports' : 'Reports'}</h1>
          <p className="mt-1 type-primary-body-b3 text-muted-foreground">{isPatient ? 'View reports for your completed bookings.' : 'Review completed bookings and uploaded reports.'}</p>
        </div>
      ...
      {/* Content */}
      {isLoading ? (
        <div className="rounded-xl border border-border bg-white p-12 text-center type-primary-body-b3 text-muted-foreground">Loading reports…</div>
      ) : isError ? (
        <div className="rounded-xl border border-border bg-white p-12 text-center type-primary-body-b3 text-destructive">Unable to load reports. Please try again.</div>
      ) : sorted.length === 0 ? (
        <div className="rounded-xl border border-border bg-white p-12 text-center type-primary-body-b3 text-muted-foreground">No reports match the selected filters.</div>
      ) : view === 'grid' ? (
              ...
              <article key={booking._id || booking.id} className="flex flex-col rounded-xl border border-border bg-white shadow-sm transition hover:shadow-md overflow-hidden">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="type-primary-body-b2 md:type-primary-body-b3 text-muted-foreground">{booking.phone}</p>
                      </div>
                    </div>
                ...
                <div className="flex flex-col flex-1 p-4 pt-3">
                  <dl className="mt-3 space-y-1.5 type-primary-body-b2 md:type-primary-body-b3">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="type-primary-label-l2 text-muted-foreground">Created At</span>
                ...
                {visibleBookings.map((booking) => {
                  ...
                  return (
                    <tr key={booking._id || booking.id} className="border-t border-border hover:bg-accent/30 transition">
                      ...
                      <td className="px-4 py-3 type-primary-body-b3 text-muted-foreground">{createdAt}</td>
                      <td className="px-4 py-3">
